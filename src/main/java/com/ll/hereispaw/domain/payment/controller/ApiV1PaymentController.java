...
public class ApiV1PaymentController {
    ...
    @Value("${custom.widgetSecretKey}")
    private String widgetSecretKey;

    // 결제 승인
    @PostMapping("/confirm")
    public GlobalResponse<Object> confirmPayment(@LoginUser MemberDto loginUser, @RequestBody PaymentRequest request) throws Exception {
        if (loginUser == null) {
...
//    }
    ...
    // 포인트 조회
    @GetMapping("/points")
    public GlobalResponse<Integer> getPoints(@LoginUser MemberDto loginUser) {
        if (loginUser == null) {
            return GlobalResponse.error(ErrorCode.ACCESS_DENIED);
        }
