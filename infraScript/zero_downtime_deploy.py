...
class ServiceManager:
    ...
    def _cleanup_dangling_images(self) -> None:
        ...
        # 먼저 이미지 목록 확인
        cmd_list = "docker images | grep '.*/here-is-paw/here_is_paw_payment' | grep '<none>'"
        ...
        # 이미지 정리
        cmd_remove = "docker images | grep '.*/here-is-paw/here_is_paw_payment' | grep '<none>' | awk '{print $3}' | xargs -r docker rmi"
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8099 -e TZ=Asia/Seoul -v /dockerProjects/here_is_paw_msa/payment/volumes/gen:/gen --pull always ghcr.io/here-is-paw/here_is_paw_payment")
    ##
